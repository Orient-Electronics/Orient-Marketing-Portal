...
reports.collect do |key, report|
	...
	corner_reports.each_with_index do |display_corner,index|
		...
		if sale.post_id.nil? and display.post_id.nil? and display_corner.post_id.nil?
			post = Post.create(:shop_id => display_corner.shop_id, :dealer_id => display_corner.shop.dealer.id, :product_category_id => display_corner.report_lines.first.product_category_id, :user_id => display_corner.user.id )
